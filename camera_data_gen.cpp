...
int* highest_point (const PointCloud::Ptr temp, pcl::PointXYZ minimum, pcl::PointXYZ maximum )
{
  ...
  int highest_z = 0;
  int* explored_vector;
	explored_vector = (int*)malloc(9 * sizeof(int));
  int j = 0;
    ...
    for (int y= 0; y < 3; y++)
    {
      ...
      for (int i = 0; i< temp->points.size(); ++i)
      {
        if(temp->points[i].x >= x0 && temp->points[i].x <= x1 && temp->points[i].y >= y0 && temp->points[i].y <= y1)
        {
          ...
          explored_vector[j] = (int) lround((highest_z - minimum.z)/m);
        }
      }
      j++;
    }
...
void automated_training_procedure(std::string object_xml_parsed)
{
	...
	// 		move_camera robot;
			...
			while (grasp_quality < quality_threshold){
		    ...
		    explored_haf_pcd = highest_point(exp_ptr,minPt,maxPt);
				...
				while(ctr_int < 9){
					std::cout<<"vector check:"<<*(explored_haf_pcd + ctr_int)<<std::endl;
					x_t_n += *(explored_haf_pcd + ctr_int) * pow(5, ctr_int);
					//std::cout<<"vector check:"<<*(unexplored_haf_pcd + ctr_int)<<std::endl;
					ctr_int += 1;
				}
				std::cout<<"vector id:"<<x_t_n<<std::endl;
				float q_reward = 0;
				...
				std::cout<<q_table[x_t][q_dir]<<std::endl;
				std::cout<<"chosen direction:"<<std::endl;
				std::cout<<q_dir<<std::endl;
	...
	out_file.open("/home/quin/catkin_ws/q_save.txt");
	for(int i = 0; i < 19683; i++){
		for(int j= 0; j < 8; j++){
			out_file<< q_table[i][j];
		}
	}
  out_file.close();
}
